...
const Matches: React.FC = () => {
  ...
  const totalPagini = Math.ceil(meciuriFiltrate.length / meciuriPerPagina);
  const indexStart = (paginaCurenta - 1) * meciuriPerPagina;
  const indexEnd = indexStart + meciuriPerPagina;
  const meciuriAfisate = meciuriFiltrate.slice(indexStart, indexEnd);
  ...
  <div className="pagination-controls">
    <button onClick={() => setPaginaCurenta(p => Math.max(1, p - 1))} disabled={paginaCurenta === 1}>
      ⟨ Înapoi
    </button>
    <span>Pagina {paginaCurenta} din {totalPagini}</span>
    <button onClick={() => setPaginaCurenta(p => Math.min(totalPagini, p + 1))} disabled={paginaCurenta === totalPagini}>
      Înainte ⟩
    </button>
  </div>
